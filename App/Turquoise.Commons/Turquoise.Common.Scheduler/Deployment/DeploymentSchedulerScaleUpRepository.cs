...
namespace Turquoise.Common.Scheduler
{
    public class DeploymentSchedulerScaleUpRepository<T> where T : new()
    {
        ...
        private ILogger<DeploymentSchedulerScaleUpRepository<T>> logger;
        private ObservableCollection<IScheduledTask<T>> items;

        public DeploymentSchedulerScaleUpRepository(ILogger<DeploymentSchedulerScaleUpRepository<T>> logger)
        {
            this.logger = logger;
